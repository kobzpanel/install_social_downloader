...
detect_os(){
  ...
  case "$OS_ID" in ubuntu|debian) ;; *) warn "This script targets Ubuntu/Debian; detected ${OS_ID}. Continuing...";; esac
}
...
preclean(){
  ...
  docker network rm "${SERVICE}_default" >/dev/null 2>&1 || true
}
...
setup_nginx(){
  ...
  # Remove stock defaults to avoid duplicate default_server on Debian/Ubuntu
...
setup_https(){
  ...
  # HTTP vhost for the domain (no cert refs yet)
...
NGINX
  ...
  nginx -t && systemctl reload nginx
  # Issue cert and add redirect to HTTPS
...
main(){
  ...
  # Pick a free host port for the container
